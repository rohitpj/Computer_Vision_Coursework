...
def prepare_test():
    ...
    model = ClassifierLayer(backbone, num_classes=20)
...
def test(name='d1'):
    ...
    backbone = new_backbone()
    model = ClassifierLayer(backbone, num_classes=20).to(device)
...
if __name__ == '__main__':
    logs = main(name='d2_20_augment_freeze_5')
    test_logs = test(name='d2_20_augment_freeze_5')
    logs = {**logs, **test_logs, 'mlp_classifer': False}

    save_logs_and_plots(logs, run_name='d2_20_augment_freeze_5')